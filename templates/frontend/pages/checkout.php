...
<script>
    window.wpStoreCheckout = function() {
        return {
            ...
            whatsappCheckoutEnabled: <?php echo !empty($whatsapp_checkout_enabled) ? 'true' : 'false'; ?>,
            ...
            checkoutViaWhatsApp() {
                ...
                if (!this.whatsappNumber) {
                    this.showToast('Nomor WhatsApp toko belum diatur.', 'error');
                    return;
                }
                const selected = this.cart.filter(i => i.selected !== false);
                ...
                selected.forEach(i => {
                    ...
                    line += ' x' + i.qty + ' = ' + this.formatPrice(i.subtotal);
                    msg += line + '\n';
                });
                msg += '\nTotal Produk: ' + this.formatPrice(this.totalSelected());
                if (this.discountAmount > 0) {
                    msg += '\nDiskon: -' + this.formatPrice(this.discountAmount);
                }
                if (this.shippingEnabled && this.shippingCost > 0) {
                    msg += '\nOngkir (' + (this.shippingCourier || '').toUpperCase() + ' ' + (this.shippingService || '') + '): ' + this.formatPrice(this.shippingCost);
                }
                msg += '\nTotal: ' + this.formatPrice(this.totalWithShipping());
                msg += '\n\nNama: ' + this.name;
                msg += '\nEmail: ' + this.email;
                msg += '\nTelepon: ' + this.phone;
                msg += '\nAlamat: ' + this.address;
                if (this.selectedSubdistrict) {
                    const sub = this.subdistricts.find(s => String(s.subdistrict_id) === String(this.selectedSubdistrict));
                    if (sub) msg += ', ' + sub.subdistrict_name;
                }
                if (this.notes) msg += '\nCatatan: ' + this.notes;
                    ...
                    <div class="wps-card">
                        <div class="wps-p-4">
                            ...
                            <div class="wps-mt-4">
                                ...
                                <div class="wps-mt-4 wps-flex wps-justify-end wps-gap-2">
                                    ...
                                    <template x-if="whatsappCheckoutEnabled">
                                        <button type="button" class="wps-btn wps-btn-secondary" :disabled="!allowSubmit" @click="checkoutViaWhatsApp()" style="background-color:#25D366;color:#fff;border-color:#25D366;">
