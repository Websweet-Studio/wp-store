...
<div>
    <button type="button"
        @click="window.wpStoreCartModal.openModal(<?php echo esc_js(wp_json_encode($product_data)); ?>)"
        :disabled="window.wpStoreCartModal?.loading"
        class="<?php echo esc_attr($btn_class); ?>"
        :style="window.wpStoreCartModal?.loading ? 'opacity:.7; pointer-events:none;' : ''">
        <template x-if="window.wpStoreCartModal?.loading">
