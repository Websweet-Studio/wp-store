(() => {
  ...
  const initProductGallery = () => {
    const galleries = document.querySelectorAll(".wps-product-gallery");
    if (!galleries.length) return;

    galleries.forEach((gallery) => {
      const zoomContainers = gallery.querySelectorAll(".wps-gallery-zoom");
      if (!zoomContainers.length) return;

      zoomContainers.forEach((zoomContainer) => {
        const mainImg = zoomContainer.querySelector("img");
        const lens = zoomContainer.querySelector(".wps-gallery-zoom-lens");
        let result = null;
        let cx = 0, cy = 0;
